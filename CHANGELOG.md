...
- 按 507 要求统一 App、Host、锁文件与 Host 握手版本为 `0.0.32`，打包构建号随版本生成；“关于”页只显示实际读取的版本，尚未取得时不再伪报旧版本。历史候选记录保留原构建号。

507 于 2026-09-08 确认本次 UI/UX 升级归属 `0.0.32`。新任务与主题、共享交互、模型连接以及项目行和文件查看已分批验证并提交，文件主动展开与收回也已完成独立验证和审查；整体验收和定版发布尚未完成；完整范围与证据见 [PRD 0030](doc/40-版本实施方案/0030-工作台-UI-UX-并行候选.md)。原多目录项目改为后续待排期。
...
- API密钥连接改为供应商行内遮蔽输入＋连接按钮，移除原生输入弹窗依赖；失败可原处重试，保存成功、取消、折叠或离开时清空。新输入通过专用保密通道交给Host，旧通用协议继续拒绝密钥，OAuth范围不变。

- 新任务归属选择移入输入卡底部；“＋”统一提供图片、文件和技能/命令，保留 `/` 快捷入口。各归属草稿独立保存，长名称省略且可查看全名，窄窗口保持模型和发送控件可用。

- 原生会话的证据查询改读 D Code Product Store，按 Task、Session、智能体及本轮执行区分来源；变更查询显示真实写入操作与单列的历史导入记录，不再依赖旧账本或虚构文件统计。

- OAuth 登录在有效流程中可重新打开授权页；自动打开或可选输入窗口失败时保留恢复入口，并区分取消、授权与保存失败。授权地址仅由 Host 保管，原生打开等待系统完成回执。

- 钥匙串后台访问默认不弹窗，连接状态改读属性，同供应商并发读取合并；拒绝或超时后停止自动重试，可在供应商旁显式授权并取消。辅助进程复用但不缓存密钥用于后续认证，保留原连接及钥匙串权限。

- 修复认证辅助窗口等待约一秒后自行退出：父进程监控移出主执行域，保留原生窗口主线程约束与父进程退出后的清理。

- OpenAI Codex 在供应商区域直接提供浏览器登录和设备码登录，移除二次方式选择窗；设备码在当前连接区域短暂展示，可取消并在完成、到期或离开时清理，授权页仍由系统浏览器打开。

- 项目行提供“更多→编辑项目”和新建任务入口，按所点项目恢复独立草稿与附件；首次发送才创建任务。新任务可以从旧文件视图回到输入，保留未保存编辑，补齐菜单与编辑表单的键盘回焦。

- 项目文件树切换到同一左侧导航，正文、预览和Git差异默认在右侧显示，中央对话及输入继续可用；跨项目浏览保留真实来源和保存目标，收起详情保留编辑缓冲，同名标签及关闭动作标明来源。

- 文件内容可主动展开占用中央空间，并通过“返回对话”收回右侧；复用同一内容和编辑缓冲，保留光标、选区、阅读位置及后台运行，恢复文件树展开/列表位置，关闭最后标签后恢复对话。
...
- 验收修复（0.0.20 提交后）：新窗口（会话前草稿）输入 `/` 面板为空——`canWrite` 以打开会话为前提在草稿态恒为 false 曾挡住整个面板，且 Composer 资源加载任务在 Host 握手前一次性错过 `readyClient` 后不再重试；面板守卫放行会话前草稿（扩展命令仍仅可写会话出现），资源加载以连接就绪为 key 重跑。
