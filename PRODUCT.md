...
- 用户从任务主对话持续与该任务的协调智能体协作：简单工作可直接完成，需要后台持续推进的工作按需交给成员；团队执行不应占住主对话。用户也可以直接与成员对话，协调者持续回收影响任务的变化、验收结论与成果。专业角色和成员人数随工作需要组织，不固定为一个执行者和一个验收者。
- 一个 Task 可承载多段持续对话。主对话由用户与协调者持续推进，也允许用户提及同任务已创建成员定向交办，协调者同时获知；运行成员由协调者创建，用户无需切换子对话才能指导成员，也不直接新建运行成员。
