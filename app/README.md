...
- `Sources/PiDCode/Models/`：协议快照、Project 元数据与 Session 精确归属、逐路径草稿、D Code 归档 / 置顶 / 会话变更账本、单根平铺 / 多根分组文件树、Git 只读查询、搜索结果、工作轮与对话导航投影、Markdown 与 Pi 图片内容呈现、工具安全 presenter 与原生消息模型。
- `Sources/PiDCode/State/`：主线程应用状态、Recent / Project 会话分页、全局置顶投影、可见会话搜索、Prompt transaction、路径草稿、完整复制、直接归档/恢复、会话级已确认变更聚合、共享观察、按需写入与冲突恢复生命周期。
- `Sources/PiDCode/Views/`：共享 surface / elevation / geometry token 与 primitive、可调会话栏 / 信息检查器的 SwiftUI 原生响应式工作台、顶部会话身份与持久重命名入口、顶部直达“会话 / 文件 / 变更”的非模态信息检查器、User Home、带置顶/归档 Hover 的 Session 导航、轮次级会话与对话导航尺、工具卡、Markdown / 代码 / Mermaid / Pi 图片内容块、Plan / 会话变更摘要、悬浮 Composer、路径谱系、复制目标、`Command-K` 搜索浮层，以及当前窗口内带页内导航、受限内容宽度、归档管理与“关于 D Code”子页的 Settings 工作台页面。
