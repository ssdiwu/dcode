...
- `Sources/PiDCode/Models/`：协议快照、Project 元数据、文件树、Git 只读查询、搜索结果与原生消息模型。
- `Sources/PiDCode/State/`：主线程应用状态、Recent/Project 会话分页、可见会话搜索、共享会话观察、按需写入与冲突恢复生命周期。
- `Sources/PiDCode/Views/`：SwiftUI 原生响应式三栏、User Home、Work Inspector、会话、Composer、`Command-K` 搜索浮层与原生 Settings 窗口。
- `Tests/PiDCodeTests/`：协议解码、搜索模型与过期结果保护、Project 持久化、文件树、Git 只读性、历史映射、Bundle 定位和状态边界测试。
