...
- `Sources/PiDCode/Models/`：协议快照、Project 元数据、逐路径草稿、D Code 归档与置顶、文件树、Git 只读查询、搜索结果、工作轮与对话导航投影、Markdown 呈现、工具安全 presenter 与原生消息模型。
- `Sources/PiDCode/State/`：主线程应用状态、Recent/Project 会话分页与置顶排序、可见会话搜索、Prompt transaction、路径草稿、完整复制、直接归档/恢复、共享观察、按需写入与冲突恢复生命周期。
- `Sources/PiDCode/Views/`：SwiftUI 原生响应式三栏、非模态 Work Inspector、User Home、带置顶/归档 Hover 的 Session 导航、轮次级会话与对话导航尺、工具卡、Composer、路径谱系、复制目标、已归档会话、`Command-K` 搜索浮层与原生 Settings 窗口。
- `Tests/PiDCodeTests/`：协议解码、搜索与过期结果保护、路径/草稿/归档/置顶、Markdown 段落和列表、Project 持久化、文件树、Git 只读性、工作轮/对话导航密度与宽度/工具/布局策略、Bundle 定位和状态边界测试。
