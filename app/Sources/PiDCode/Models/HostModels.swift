...
enum HostCompatibilityError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        ...
        case let .unsupportedProtocol(version):
            "D Code 0.0.11 不支持 Host Protocol \(version)。请重新构建并使用同一版本的 App 与 Host。"
        case let .incompatibleHostVersion(version):
            "当前 Host 版本为 \(version ?? "未知")，D Code App 需要 0.0.11。请重新构建 App，避免混用旧 Host。"
        case let .missingCapabilities(capabilities):
            "当前 Host 缺少 0.0.11 必需能力：\(capabilities.joined(separator: "、"))。D Code 已停止连接，以免错误读取或写入会话。"
        }
    }
}

enum HostCompatibility {
    static let appVersion = "0.0.11"
