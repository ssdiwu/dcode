...
- 自动派发先确定每个子智能体的有序模型回退链，再核验启用、访问、任务能力与适用配额；有效剩余额度高于设置的门槛才可自动选用，不高于门槛时沿链跳过，同池影响一起处理。门槛由 Host Product Store 持久化，设置→模型可选整数 1–30%，默认 1%；修改仅影响后续评估，不重排回退链或强制中断正在执行的工作。未知、过期和查询失败不能冒充可用。配额查询和候选回退优先复用本机 `pi-dusage`、`pi-dteam` 已有能力，按 D Code 产品合同适配；精确状态、回退与验证由 PRD 0029 拥有。
...
- 不记录、复制或回显已存 API key、OAuth token、认证文件正文等凭据。507 明确要求的 API Key 行内连接可在遮蔽输入框中临时接收当次输入，经可信主 frame 的专用保密 IPC 与 Host 私有管道提交；不得进入通用协议记录、壳配置、页面持久化、日志、会话或模型上下文。成功/取消/折叠/离开时清空，失败仅在当前可见输入框保留以供重试；持久化仍由 Host 写入钥匙串。
...
- 本机候选使用 `npm --prefix client run dist`，由 `client/scripts/package.mjs` 构建 Host、装配锁定生产依赖与许可证，输出独立的未签名 Electron 应用；不得覆盖正在运行的候选。正式发布前须工作区干净、核对各组件 manifest/锁文件与本次发布版本，并另行完成签名、公证、分发和数据回退检查；当前候选脚本不代表这些发布检查已经执行。
