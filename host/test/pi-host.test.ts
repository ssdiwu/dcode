...
test("host lists, inspects, and opens with immediate takeover", async () => {
  ...
  try {
    ...
    assert.equal(hello.hostVersion, "0.0.32");
    assert.equal(hello.piVersion, "0.85.1");
    ...
    assert.equal((hello.capabilities as Record<string, boolean>).taskPlanWorkList, true, "任务计划与工作清单必须在 hello 能力位可发现");
