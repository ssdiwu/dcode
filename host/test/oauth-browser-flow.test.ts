...
test('other providers retain native selection failure and deliberate cancellation states',async()=>{
 for(const mode of ['failure','cancel']){const f=await fixture();try{f.native.prompt=async()=>{if(mode==='cancel')throw authCancelled();throw Error('private helper diagnostics');};const base=f.runtime.getProvider('openai-codex')!;f.runtime.registerNativeProvider({...base,id:'other-oauth'});await f.manager.start('other-oauth','oauth','select-'+mode);await f.manager.idle();const v=(await f.manager.get()).providers.find(p=>p.providerId==='other-oauth')!;assert.equal(v.state,mode==='cancel'?'cancelled':'failed');assert.equal(v.failureCode,mode==='cancel'?undefined:'interaction_unavailable');assert.ok(!JSON.stringify({events:f.events,v}).includes('private'));}finally{await f.close();}}
});
