...
test(
  ...
  async () => {
    ...
    try {
      ...
      const managed=await work.mutateStore("project.create",{title:"界面目录维护",directory:projectSource});await waitFor(()=>assert.ok(screen.getByRole("button",{name:"更多项目操作 界面目录维护"})));
      fireEvent.keyDown(screen.getByRole("button",{name:"更多项目操作 界面目录维护"}),{key:"Enter"});fireEvent.click(await screen.findByRole("menuitem",{name:"编辑项目"}));fireEvent.change(screen.getByRole("textbox",{name:"项目名称"}),{target:{value:"已修改的项目名称"}});fireEvent.click(screen.getByRole("button",{name:"保存项目"}));await waitFor(()=>assert.ok(!screen.queryByRole("dialog",{name:"编辑项目"})));
      assert.equal((await host.handle("foundation.snapshot",{})).projects.find(project=>project.id===managed.project.id).title,"已修改的项目名称");
      fireEvent.keyDown(screen.getByRole("button",{name:"更多项目操作 已修改的项目名称"}),{key:"Enter"});fireEvent.click(await screen.findByRole("menuitem",{name:"编辑项目"}));window.dcode.chooseDirectory=async()=>projectTarget;fireEvent.click(screen.getByRole("button",{name:"项目文件夹"}));await waitFor(()=>assert.ok(screen.getByRole("checkbox",{name:"同时移动项目文件"})));fireEvent.click(screen.getByRole("checkbox",{name:"同时移动项目文件"}));fireEvent.click(screen.getByRole("button",{name:"保存项目"}));await waitFor(()=>assert.ok(!screen.queryByRole("dialog",{name:"编辑项目"})),{timeout:10000});
