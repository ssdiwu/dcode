...
const stats={browsers:0,manuals:0,selects:0,writes:0},vault=new Map();let authUrl='';
...
globalThis.fetch=async input=>{if(String(input)==='https://auth.openai.com/oauth/token')return Response.json({access_token:${JSON.stringify(access)},refresh_token:${JSON.stringify(refresh)},expires_in:3600});throw Error('Unexpected fixture network');};
...
app.once('browser-window-created',(_event,win)=>win.webContents.once('did-finish-load',async()=>{
 ...
 const inside=label=>run('Array.from('+box+'.querySelectorAll("button")).find(b=>b.textContent.trim()==='+JSON.stringify(label)+').click()');
 ...
 try{
  ...
  await until(box,'provider');await inside('登录 ChatGPT');await until(box+'.textContent.includes("未能自动打开")','recoverable browser error');assert.equal((await stats()).manuals,0);
  ...
  await inside('重新登录');await until('!!'+box+'.querySelector("[aria-label=打开登录页面]")','new flow');await inside('取消连接');await until(box+'.textContent.includes("已取消连接")','explicit cancellation');assert.equal((await stats()).writes,1);
  await fs.writeFile(${JSON.stringify(join(temp,'result.json'))},JSON.stringify({passed:true,actualHost:true,actualSdk:true,publicFlowControls:true,statusDuringOpening:true,browserRetry:true,manualFailureRecoverable:true,manualStartsOnlyOnClick:true,cancellation:true,secretProjectionAbsent:true,nativeOperations:'controlled; OS opener verified separately'}));app.quit();
...
for(const name of ['ELECTRON_RUN_AS_NODE','DCODE_RENDERER_URL','DCODE_THEME','DCODE_CAPTURE','DCODE_CREDENTIAL_PIPE_FD'])delete env[name];
...
const scan=async path=>{for(const e of await readdir(path,{withFileTypes:true})){const p=join(path,e.name);if(e.isDirectory())await scan(p);else if(e.isFile()){const bytes=await readFile(p);for(const value of [access,refresh,'ui-private-code','code_challenge='])assert.ok(!bytes.includes(value),'Private flow data persisted');}}};await scan(join(temp,'.dcode'));await scan(join(temp,'profile'));for(const value of [access,refresh,'ui-private-code'])assert.ok(!(output.stdout+output.stderr).includes(value));
