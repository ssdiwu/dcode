...
app.once('browser-window-created',(_event,win)=>win.webContents.once('did-finish-load',async()=>{
 ...
 const click=async label=>{await until('Array.from(document.querySelectorAll("button")).some(b=>!b.disabled&&(b.getAttribute("aria-label")==='+JSON.stringify(label)+'||b.textContent.trim()==='+JSON.stringify(label)+'))','button '+label);return run('Array.from(document.querySelectorAll("button")).find(b=>b.getAttribute("aria-label")==='+JSON.stringify(label)+'||b.textContent.trim()==='+JSON.stringify(label)+').click()');};
 ...
 try{
  ...
  await click('文件与 Git');await until('!!document.querySelector(".files-workspace")','project file view');await click('note.md');await until('!!document.querySelector(".file-markdown")','document');await click('编辑');
  ...
  await click('文件与 Git');await until('document.querySelector(".file-editor")?.value===\"# Unsaved A\"','same buffer retained');
  ...
  await key('更多项目操作 '+a);await until('!!document.querySelector("[role=menuitem]")','menu over preview');for(let i=0;i<100&&previewVisible();i++)await sleep(20);assert.equal(previewVisible(),false);
  ...
  await draft(a,'');await click('文件与 Git');await until('!!document.querySelector(".file-tab-strip")','return to buffers');await click('关闭文件 note.md');await until('!!document.querySelector("[role=dialog]")','dirty close');await click('放弃更改');
