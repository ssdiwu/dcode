...
export function ExecutionProcess({turn}:{turn:ExecutionTurn}) {
  ...
  const label = turn.status === "running" ? "正在执行" : turn.status === "error" ? "执行失败" : turn.status === "aborted" ? "已停止" : turn.status === "interrupted" ? "执行已中断" : turn.status === "unknown" ? "状态待核对" : "执行完成";
  ...
  return <details className="execution-process" data-state={turn.status}>
    ...
    <div className="execution-steps">
      {turn.steps.length ? turn.steps.map(step=><section className={`execution-step ${step.kind}`} key={step.id}>
        <div className="execution-step-label">{step.state && <ExecutionStatusIcon state={step.state}/>}<span className="execution-step-title">{step.title}</span>{step.state && <span>{step.state === "running" ? "进行中" : step.state === "error" ? "失败" : step.state === "unknown" ? "待核对" : "已完成"}</span>}</div>
