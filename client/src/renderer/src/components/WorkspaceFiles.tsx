import {useFileViewState} from "../workbench/useFileViewState";
...
function Tree({source,path="",onOpen,expanded,onToggle}:{source:WorkspaceSource;path?:string;onOpen:(path:string)=>void;expanded:string[];onToggle:(path:string)=>void}){
  const [tree,setTree]=useState<WorkspaceTree|null>(null),[error,setError]=useState("");
  ...
  return <ul className="file-tree" aria-busy={!tree&&!error}>{error&&<li role="alert">{error}</li>}{!tree&&!error&&<li className="secondary">正在读取…</li>}{tree?.entries.map(entry=><li key={entry.relativePath}>
    <button className="file-tree-row" disabled={entry.kind==="link"||entry.kind==="other"} title={entry.kind==="link"?"符号链接不进入项目预览":entry.name} onClick={()=>entry.kind==="directory"?onToggle(entry.relativePath):onOpen(entry.relativePath)}>
    ...
    </button>{entry.kind==="directory"&&expanded.includes(entry.relativePath)&&<Tree source={source} path={entry.relativePath} onOpen={onOpen} expanded={expanded} onToggle={onToggle}/>}
...
function FileBody({tab,model,overlay}:{tab:FileTab;model:WorkspaceFileModel;overlay:boolean}){
  const editor=useRef<HTMLTextAreaElement>(null),preview=useRef<HTMLDivElement>(null);const [lineText,setLineText]=useState(String(tab.line??1));const body=tab.draft??tab.document?.text??"";const count=body.split("\n").length;
  useEffect(()=>setLineText(String(tab.line??1)),[tab.line]);
  useFileViewState(editor,tab,"editor",model);
  useFileViewState(preview,tab,"preview",model);
  ...
  return <div className="file-content">
    ...
    <div className={html?"html-split":"file-document"}>
      {tab.document.kind==="image"?<div className="file-image" ref={preview}><img src={tab.document.dataUrl} alt={tab.path}/></div>:tab.mode==="preview"&&tab.document.kind==="markdown"?<FileReferenceContext.Provider value={reference=>model.openRelative(tab,reference)}><div className="file-markdown" ref={preview}><Markdown text={body}/></div></FileReferenceContext.Provider>:<textarea wrap="off" className="file-editor" ref={editor} aria-label={editing?"文件编辑内容":"文件原文"} readOnly={!editing||tab.saving} spellCheck={false} value={body} onChange={event=>model.patch(tab.id,{draft:event.target.value})} onKeyDown={event=>{if(editing&&(event.metaKey||event.ctrlKey)&&event.key==="s"){event.preventDefault();void model.save(tab.id);}}}/>}
...
export function WorkspaceFileNavigation({model}:{model:WorkspaceFileModel}){
  ...
  const list=useRef<HTMLDivElement>(null),listing=useRef<HTMLDivElement>(null);
  useLayoutEffect(()=>{
    const element=list.current,content=listing.current;if(!element||!content)return;
    const id=`navigation:${model.scope}:${model.navigationPath}:${model.navigationView}`,saved=model.readView(id,"list");
    let restoring=!!saved;
    const capture=()=>{if(!restoring&&element.isConnected)model.writeView(id,"list",{top:element.scrollTop,left:element.scrollLeft,lineRevision:0});};
    const restore=()=>{if(restoring&&info&&!content.querySelector('[aria-busy="true"]')){element.scrollTop=saved!.top;element.scrollLeft=saved!.left;restoring=false;}};
    const userMove=()=>{restoring=false;};
    const observer=new ResizeObserver(restore);observer.observe(content);restore();
    element.addEventListener("scroll",capture);element.addEventListener("wheel",userMove);element.addEventListener("pointerdown",userMove);
    return()=>{capture();observer.disconnect();element.removeEventListener("scroll",capture);element.removeEventListener("wheel",userMove);element.removeEventListener("pointerdown",userMove);};
  },[model.scope,model.navigationPath,model.navigationView,info?.root]);
  ...
  return <section className="file-navigation" aria-label="项目文件导航">
    ...
    <div className="file-navigation-list" ref={list}><div ref={listing}>
    {model.navigationPath&&<p className="file-navigation-path" title={model.navigationPath}>{model.navigationPath}</p>}
    {error&&<p className="inline-error" role="alert">{error}</p>}
    {info&&model.source&&model.navigationView==="files"&&<Tree key={`${model.scope}:${model.navigationPath}:${revision}`} source={model.source} path={model.navigationPath} onOpen={path=>model.open(path)} expanded={model.treeExpanded} onToggle={model.toggleDirectory}/>}
...
function DiffBody({tab,model,work}:{tab:FileTab;model:WorkspaceFileModel;work:Workbench}){
  const viewport=useRef<HTMLDivElement>(null);useFileViewState(viewport,tab,"diff",model);
...
export function WorkspaceFiles({model,work,overlay=false}:{model:WorkspaceFileModel;work:Workbench;overlay?:boolean}){
  ...
  useEffect(()=>{strip.current?.querySelector<HTMLElement>('[aria-selected="true"]')?.scrollIntoView({block:"nearest",inline:"nearest"});},[model.active?.id,model.visible,model.expanded]);
