...
export function WorkspaceFiles({model,work,overlay=false}:{model:WorkspaceFileModel;work:Workbench;overlay?:boolean}){
  ...
  useEffect(()=>()=>{diffRequest.current++;},[]);
  const showDiff=async(path:string,staged=false)=>{
    const request=++diffRequest.current,currentScope=model.scope;setError("");
    try{const result=await api().request<NonNullable<typeof diff>>("workspace.diff",{source:model.source,path,staged});if(request===diffRequest.current&&scope.current===currentScope)setDiff(result);}
  ...
  return <section className="files-workspace" aria-label="文件与 Git">
    ...
    </aside><div className="file-main">{error&&<p className="inline-error" role="alert">{error}</p>}{diff?<><div className="file-toolbar"><strong>{diff.path}</strong><button className="text-button" onClick={()=>void showDiff(diff.path,!diff.staged)}>{diff.staged?"查看工作区改动":"查看暂存改动"}</button><button className="text-button" onClick={()=>{model.open(diff.path);setDiff(null);}}>打开文件</button><button className="text-button" onClick={()=>work.updateDraft(work.draftKey,previous=>({...previous,text:`${previous.text}${previous.text?"\n\n":""}[${diff.path}](<${encodeURI(diff.absolutePath)}>)（${diff.staged?"暂存改动":"工作区改动"}）\n\`\`\`diff\n${diff.diff.slice(0,12000)}${diff.diff.length>12000?"\n（差异较长，仅引用开头部分）":""}\n\`\`\`\n`}))}>引用差异</button></div><div className="git-diff" aria-label="文件差异">{diff.diff?diffLines(diff.diff).map((line,index)=><button key={index} className={`diff-line ${line.kind}`} disabled={line.kind==="header"} aria-label={line.kind==="header"?undefined:`引用差异${line.kind==="remove"?"旧":"新"}侧第 ${line.newLine??line.oldLine} 行`} onClick={()=>work.updateDraft(work.draftKey,previous=>({...previous,text:`${previous.text}${previous.text?"\n\n":""}[${diff.path}](<${encodeURI(diff.absolutePath)}>)（${diff.staged?"暂存差异":"工作区差异"}，${line.newLine!==undefined?`新侧第 ${line.newLine} 行`:`旧侧第 ${line.oldLine} 行`}）\n差异版本：${diff.digest}\n\`\`\`diff\n${line.hunk}\n${line.text}\n\`\`\`\n`}))}><span className="diff-number">{line.oldLine??""}</span><span className="diff-number">{line.newLine??""}</span><span>{line.text}</span></button>):"这一侧没有改动。"}</div></>:model.active?<FileBody key={model.active.id} tab={model.active} model={model} overlay={overlay||!!model.closing}/>:<p className="files-empty secondary">选择左侧文件或 Git 改动。编辑仅支持 Markdown 和 HTML。</p>}</div></div>
  </section>;
}
