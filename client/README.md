...
- 模型保留完整目录、认证态、启用范围及默认选择；旧供应商可显式接管非敏感配置并保留认证引用，API密钥在供应商行内填写并连接，OAuth继续使用浏览器登录，环境变量引用保留为高级选择。新输入仅短暂存在于遮蔽框和专用保密提交中，不回显已存凭据，不进入通用请求记录或页面持久化。旧 Swift 的界面偏好单向继承，已保存的新偏好优先。
