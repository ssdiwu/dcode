...
项目文件查看默认使用同一左侧导航与右侧信息检查器。`WorkspaceFileNavigation` 负责目录/Git列表，`WorkspaceFiles` 负责真实内容标签；中央Transcript及Composer不因打开文件被卸载。`useWorkspaceFiles` 保存独立的浏览来源和按来源标识的标签/缓冲，任务与其Project目录归一为同源；保存、差异及相对链接始终使用标签自己的来源。返回任务只切左栏，收起详情不丢缓冲，关闭脏标签仍确认。读取序号保护迟到引用及旧重载；显式文件导航请求会恢复隐藏左栏。`test/main/file-inspector.mjs` 使用隔离目录和真实本地SSE响应验证A运行时浏览/保存B、A写保护、正文/差异/HTML及原对话输入并存。主动展开已使用同一面板和当前运行期阅读恢复，具体范围见下。

文件面板的“展开文件内容”只改变同一节点的显示空间，左栏保留，原对话维持并列时的几何并暂时不可交互；“返回对话”收回右侧并恢复输入焦点。`useFileViewState` 按标签保存滚动、光标/选区和Markdown可见文本锚点，`useWorkspaceFiles` 仍是唯一编辑缓冲与来源所有者；阅读数据只在当前运行期内存中保留，关闭标签清理。编辑原文使用不自动折行的逻辑行和横向滚动，使行定位及展开前后位置一致；正文与保存合同未改。显式行定位带独立递增标记，切换面板或标签不会重放旧定位。文件树展开与列表位置可从设置返回时恢复。

复验：完成client构建后，从仓库根运行 `node client/test/main/file-inspector.mjs --expand`。独立窗口检查同面板/HTML实例、编辑与预览阅读恢复、差异、目录导航、展开内换标签、全局新任务、关闭最后标签、宽窄/深浅/三档字号及减少动态效果；使用隔离本地SSE和测试文件，不操作用户账户/应用。HTML跨展开与收回保持实例；切换文件及已有设置页生命周期的隔离策略继续沿用，未新增页面表单或未保存正文的进程退出恢复承诺。
