...
The workbench has two persistent structural regions: Navigation Sidebar（导航栏；当前实现名会话栏）on the left and Main Workspace（主工作区）in the center. Reading content, its empty state and the Composer share one centered column while the Task HUD（任务浮层）is closed. Opening the HUD moves that column left to make room for an inset, independently rounded card at the upper right; closing it restores the centered layout. Updates inside an already visible HUD do not move the reading column. Only when width is insufficient may it overlap the reading area and gain collapse / reopen controls. Information Inspector（信息检查器）appears as a full-height right rail only when the user opens a file, Artifact, Diff or another object that needs sustained inspection; the reading column centers in the space remaining beside a docked inspector. Settings（设置）and Archived Sessions（已归档会话）remain Workbench Pages（工作台页面）inside the Main Workspace.

Project file browsing reuses these regions: the project row exposes separate management, file browsing and new-task actions. File browsing temporarily switches the Navigation Sidebar to the selected project’s file tree, with a return-to-tasks action. Selecting a file opens its content in the Information Inspector by default while the current conversation and Composer remain available. Only an explicit expand action gives that content more space in the Main Workspace; returning restores the same conversation and editing state. Browsing never changes task ownership or implicitly adds model context. The confirmed behavior and acceptance scope are owned by [the 0.0.32 workbench PRD](doc/40-版本实施方案/0030-工作台-UI-UX-并行候选.md).
