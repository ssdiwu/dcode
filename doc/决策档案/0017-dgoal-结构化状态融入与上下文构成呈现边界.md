...
状态：当前实现有效，未来产品边界已取代（当前 `0.0.8+` 的 dgoal 读取与呈现边界继续有效；未来 Goal（目标）/ Plan（计划）产品所有权由 [ADR 0035](0035-项目任务主干与任务拥有多会话边界.md) 与 [ADR 0036](0036-能力模块分层与-Pi-拓展吸收边界.md) 取代，不再以 dgoal 为运行时依赖）
