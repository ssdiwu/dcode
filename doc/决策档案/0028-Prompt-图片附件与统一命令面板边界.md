...
状态：Partially Superseded（ADR 0030 取代 `+` 仅附件与资源只经 `/` 触达的入口决定；ADR 0034 取代“不做剪贴板粘贴图片”与 Skill 调用名直接展示的决定；图片附件、队列、预填与缓冲区边界继续有效）
...
8. **不做**：音频 / 视频 / PDF 等非图片附件直传（协议未核实）、附件进入 follow-up 队列、附件持久化或草稿化、拖拽图片、行内缩略图渲染（以 chip：文件名 + 体积 + 移除呈现）、自动压缩或转码图片。剪贴板粘贴图片后来由 ADR 0034 纳入同一附件链路。
