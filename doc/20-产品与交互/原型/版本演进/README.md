...
状态：Confirmed Direction（已确认方向；`v0.0.1`–`v0.0.16` 已推送源码标签，`0.0.17`–`0.0.19` 已实现待人工验收打标；`0.0.20`–`0.0.24` 于 2026-08-22 由 507 按首个 dogfood 反馈与既有缺口预填）
...
| `0.0.25+` | 只处理连续 dogfood（自用）暴露出的真实闭环缺口；每个版本只增加一个可验证能力。 | 为赶 `0.1.0` 预装空入口或一次塞入多个大系统。 | 缺口有真实复现、明确版本差异与独立验收；版本数量按事实扩展。 |
