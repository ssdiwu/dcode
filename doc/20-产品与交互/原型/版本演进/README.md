...
- `0.0.11` 已由 [ADR 0020](../../../决策档案/0020-Git-差异事实来源与只读边界.md)固定 Git 差异事实来源（Swift 只读直跑 git、双轨诚实、截断护栏）。`0.0.12` 已由 [ADR 0021](../../../决策档案/0021-验证证据身份与账本边界.md)固定验证证据身份（真实工具执行入账、退出推导三态、revision 异步补全、非门禁）。`0.0.13–0.0.14` 先固定 App 替换 / 回滚与重启恢复协议，避免自更新破坏当前可用基线。
