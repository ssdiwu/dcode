...
状态：Confirmed Direction（已确认方向；`v0.0.1`–`v0.0.16` 与 `v0.0.25` 已推送源码标签；`0.0.17`–`0.0.20` 已实现但仍保留各自人工验收边界；`0.0.25` 自动回归通过，原生人工验收待完成）
...
| `0.0.25` | Project 目录一一对应与 Composer 收口：主页只选择 Project；改目录时保持 Session 身份直接迁移 `cwd`，文件迁移为第二步选择；输入区只保留 `+`、推理强度与发送，文件、Skill、命令、目标与计划收进 `+`。 | 多目录 Project、目录迁移伪装成复制、模型选择器 / 插件 / 网页预览占据 Composer 一级。 | 项目、目录与历史会话的归属一致；目录迁移的 Session 与文件影响可独立核对；输入区一级只承载当前任务。 |
| `0.0.26+` | 只处理连续 dogfood（自用）暴露出的真实闭环缺口；每个版本只增加一个可验证能力。 | 为赶 `0.1.0` 预装空入口或一次塞入多个大系统。 | 缺口有真实复现、明确版本差异与独立验收；版本数量按事实扩展。 |
