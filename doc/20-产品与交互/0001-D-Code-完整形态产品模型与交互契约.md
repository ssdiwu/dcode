...
- Task 拥有一份 Plan 和一份 Work List；Work Item 保留状态、依赖、交付物和必要证据，并可由同任务的独立成员执行和验收。只有需要独立任务身份、目标和验收生命周期时才提升为 Subtask，独立成员、子对话或产物本身不是提升条件。
...
- 任务内自适应协作的完整行为与验收由 [PRD 0029](../40-版本实施方案/0029-任务内自适应协作产品需求.md)拥有，进程锚点由 [ADR 0045](../决策档案/0045-任务内自适应协调与智能体进程边界.md)固定；以下是跨版本产品关系，不是实现完成声明。
- Agent Team 属于 Task。Task 可以直接启动单个 Agent Run，也可以显式启动一轮 Team Run；团队、会话和模型都不拥有任务。
- 交互 Task 从首次对话起由该任务的 Coordinator Agent 承接；它是活在 Coordination Session 中的主智能体角色，LLM 只是其执行使用的模型。简单工作可以直接处理，需要后台持续推进、独立分工或独立验收时自主组织成员并专注协调，不更换任务或主对话。导航中点击 Task 主行即进入任务对话，协调会话不显示为独立的子会话行；展开 Task 只列实际启动的 Child Agent Session，返回任务对话不会停止其运行。
...
- Team Run 必须在明确目标、范围、完成信号和停止边界下有真实、可追踪的启动动作；该动作可以由协调者在已有任务授权内自主发起，不要求每次增加用户组队确认。后台工作即使只有一项也可以派发；成员职责、数量和串并行关系按需组织，不固定一名执行者与一名验收者。
...
- 用户可以直接与子智能体对话，其他成员继续工作；影响任务范围、进度、结论或依赖的变化以有来源的事件回流协调者，避免其继续使用旧安排。后台执行期间主对话仍可处理新消息，不等待整批成员结束。用户可见的一个任务可以包含多个工作项和子对话，不把这些子工作自动升为顶层任务。
- 主对话也支持 `@` 同任务已创建成员的定向交流，不要求用户先进入子对话。消息保留发送者、所在对话与目标成员，协调者同时获知指令和变更；成员回复回到对应对话并保留真实来源。成员只能由任务协调者创建；用户直接交办已有成员不等于新建成员，提及不存在的名字不得隐式创建，Profile 编辑仍只是配置。
- 协调 Agent 负责派发、判断、冲突、用户问题去重与最终综合；成员负责有界工作并以 Finding、Request、Report 和 Evidence 协作。需要独立验收时由不同执行实例承担，协调者再复核：成果有问题则安排执行返工，验收不足则安排重查。成员间任意自由通信和自动合并不因自主组队而自动成立。
- 每个活动 Agent Run 以一个独立的 Agent Execution Process 为主执行锚点，可归属多个工具辅助进程；不同智能体不得仅以同进程 Runtime 区分，同一智能体同时只保留一个主执行锚点。工作收口后可以短暂保留空闲进程，再安全回收；再次需要时从持久化上下文启动新进程，不要求永久常驻，也不把恢复上下文说成恢复旧进程。
...
- [ ] Agent Team 在 Task 授权内由协调者通过真实创建与启动动作组织；用户可请求协调者组队，但不能直接创建运行成员。短任务仍可保持单 Agent；团队概览显示成员职责、实际模型与工具、声明范围、运行状态、活动、耗时 / 用量、请求、报告和证据，不暴露隐藏推理、完整私有过程或凭据。
...
- [ ] 主对话持续响应、按需多执行与多验收成员、用户对子对话的变更回流、独立主执行进程与辅助进程归属、空闲回收及上下文重启、验收返工和协调复核按 [PRD 0029](../40-版本实施方案/0029-任务内自适应协作产品需求.md)分别通过完整路径验收，不能由旧团队流程或文档声明代替。
