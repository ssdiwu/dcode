...
Pi 已经拥有真实会话、分支路径、复制会话、配置、模型和工具运行能力，但这些对象在原生桌面工作流中仍缺少稳定的信息层级。既有扩展也沉淀了 Goal、团队与通信等有价值的机制，但它们不是 D Code 的产品对象或界面基座。仅按裸 `cwd` 平铺会话会把工作上下文暴露成目录选择，无法同时满足快速恢复最近工作、查找完整历史、观察 Agent Team 和检查项目文件的需求。
...
- User Home 提供 D Code 创建的 Recent Sessions、全局搜索与全局新建会话入口。创建前必须已有 Project；主页下挂条只让用户选择本次工作的 Project，Project 持有的唯一目录成为新会话 `cwd`。User Home 本身仍不是 Project，也不拥有项目文件。
...
- Project 是 D Code 自己定义的工作组织对象，不等于 Pi Session；它拥有名称和唯一的 Project Directory（项目目录），不拥有 Pi Session 的消息、模型上下文或 Artifact。
- 每个 Project 恰有一个规范化、可访问的项目目录；每个规范化目录最多属于一个 Project。目录不能重复登记、静默抢占或作为第二个目录附加给同一 Project。
- Project 会话仅投影 Header `cwd` 与项目目录精确相同的 Pi Session。普通行只常驻单行标题；Project、项目目录、完整 `cwd` 与当前 Git 分支在悬停或键盘聚焦时显示。置顶对象在该 Project 已成立的候选集合内优先，其余按更新时间排列；初始呈现前 10 条，再通过“查看更多”逐批显示更早记录。
- 编辑 Project 并改目录时，D Code 在明确操作下直接迁移精确匹配会话的 Header `cwd`，保留 Session ID、历史、文件路径与谱系。是否迁移目录内文件是默认关闭的第二步选择，不能自动合并或覆盖文件。

### Project Directory（项目目录）

- Project Directory 是用户在创建或编辑 Project 时选择的确切目录。
- 项目目录与 Pi Session 的规范化 `cwd` 相同才形成关联；不会因父目录已登记而静默吸收任意后代目录的会话。
- Project 改目录不是 Session Copy：迁移不创建新 Session ID，也不把原会话变成副本。活动、租约不稳定或 Header 变化的会话必须阻止迁移。

### Pi Session（Pi 会话）与项目投影

- Pi Session 的稳定 ID、消息、模型上下文和 Pi 已持久化的运行条目继续以 `~/.pi/agent` 为唯一权威。
- 创建 Project 后，D Code 读取并展示其项目目录精确匹配的既有 `cwd` 会话；不需要导入或复制 JSONL。只有用户在编辑 Project 时明确改目录，Host 才能受控改写关联 Session Header 的 `cwd`。
- 未关联的旧 Pi Session 不出现在 Recent 或任何 Project；打开、续写或观察旧会话不会把它转换为 D Code Recent Session。
- 在 Project 内或主页新建会话时，直接以所选 Project 的唯一目录开始会话前草稿；首次发送后创建的 Pi Session 使用该目录作为 `cwd`。
...
- 草稿不按时间自动过期，也不会因退出应用、切换会话或路径、移除 D Code Project 或迁移 Project 目录而清除。只有对应内容发送成功、用户明确清空草稿（取消新路径草稿属于明确清空），或对应 Pi Session 被实际删除时，才清除相应本机草稿；删除一个草稿不得波及同一会话的其他路径草稿。
- 独立 Session 操作使用“复制到项目…”与“复制到项目并归档原会话…”。二者都把源 Session 的全部已持久化历史与路径复制到所选 Project Directory，创建新的 Session ID 与目标 `cwd`；源 Session、未发送草稿、项目文件和 Git 状态不变。
...
- Project 主行与展开/收起控件分开：点击 Project 主行直接进入 Project 作用域并显示 Files，无需先选择任何 Session；展开后显示其项目目录精确匹配的 Session。普通会话行只常驻显示单行标题；悬停或键盘聚焦后，非模态详情展示完整标题、更新时间、Project、项目目录、完整 `cwd` 与当前 Git 分支。这里的分支是工作目录当前状态，不冒充会话历史分支。
...
- Composer 控制行只保留 `+`、独立推理强度和发送；已创建 Session 可按需显示上下文圆环。模型名、CPU 图标、模型下拉与极速开关不进入 Composer；模型设置属于 Settings，当前模型事实属于会话运行信息。上下文占用可展开查看已用量与总容量，但已加载来源仍由 Session 的 Context 面板负责。
- 当前 Session Run 正在执行且没有等待结构化用户输入时，Composer 仍接受普通文本作为绑定当前 Session / Path 的 Queue Item。队列以 Composer 附近的最小表面显示顺序与待派发事实；派发前可查看、编辑、调整顺序或撤回，且不会进入 transcript、Pi Session 或模型上下文。
...
- Composer 的 `+` 承载“发送前加入本次请求”的 Command、Skill 与 Prompt Template，选择后只把可见 `/…` 调用预填进草稿，不自动发送；`/` 面板继续支持键盘命令选择，Settings 承载跨请求持久配置与资源生命周期。
- Settings 的模型页面复用 Pi 当前 Model Catalog（模型目录），按供应商展示模型、在线刷新 / 本机缓存状态、Pi 认证状态、默认模型以及“在 D Code 中启用”状态。Pi 原生 `enabledModels` 只定义模型循环规则；D Code 明确复用同一组规则约束 Composer 模型选择器与快捷切换，不把它外推成供应商或模型可用性。`0.0.7` 只编辑全局 Pi 设置，当前 `cwd` 的项目覆盖只读标明；启停不会强制改写当前 Session 或历史消息记录的实际模型。
...
- Project 作用域包含 Files 与 Changes；Files 直接展示唯一项目目录的根文件树，Changes 只呈现当前 Git 事实。
...
- 会话栏与信息检查器默认均为 `400 pt`，可在 `400–520 pt` 内调整；左栏由此为 Project 名称、创建 / 展开动作与后续 Activity View 保留稳定宽度，右栏使标准字号下的完整 Session ID 保持单行。旧于下限的已保存宽度自动校准。宽度持久保存，双击调整把手恢复默认，键盘和 VoiceOver 有等价调整方式。调整边缘本身不绘制常驻或焦点竖条，指针移入后只切换为水平调宽光标并保持可拖动。
...
- 全局搜索只覆盖“D Code 创建的 Recent Sessions”与“已关联 Project Directory 投影出的会话”的并集，而不只覆盖当前已经显示的 10 条摘要。
- 可搜索会话标题、用户消息正文和助手消息正文。
- 默认不搜索隐藏 thinking、原始工具输入、工具结果、认证内容或其他可能含凭据的非对话正文。
- 结果显示命中片段、会话名称、更新时间、`cwd`、所属 Project 与项目目录；可按 Project 缩小范围，不另暴露目录筛选器。
- 搜索命中未加载的旧会话时，选择结果会打开对应稳定 Session ID 并定位相关消息。
- 搜索索引只能是可从 Pi 会话重建的本地缓存；索引缺失或损坏不能改变 Pi Session，也不能伪装成“没有历史”。

## Agent Team（智能体团队）

- Agent Profile 是具有稳定身份和版本的可复用本机定义，包含名称、职责说明、可选默认工作范围、模型路由与版本化 Skill 引用。创建、编辑或选择档案不会启动 Worker，也不会创建 Pi Session；编辑只影响未来运行。声明范围只服务调度与验收，不是运行时 Permission Grant。
...
- 每个 Team Member Run 保存所用 Profile 版本、当次任务、实际模型、实际工具集合、Skill 身份 / 版本、声明写入范围、来源上下文、子会话与执行目录引用、queued / running / waiting / completed / failed 状态、可见事件与结构化 Report；对应子 Pi Session 可以独立持久化，背后的模型生成、工具调用与 Worker Runtime 只属于 Host 运行期。
...
- 信息检查器负责团队概览、成员选择、当次职责、实际模型与工具集合、运行状态、当前活动、耗时 / 用量、卡住与重试；主工作区负责所选成员的 Finding、Request、控制记录、Verifier 退回原因与结构化 Report。
...
- Agent Profile 或 Team Member Run 可以记录声明的职责、工具集合与预期写入范围，供调度、报告和验收核对；这些声明不是运行时权限授予，也不得冒充操作系统级文件沙箱或 D Code 工具门禁。
- 动作权限固定完全访问（ADR 0023）：bash、文件写入与自定义工具静默执行，不产生请求、询问或权限审计，与 Pi CLI 行为一致；不存在模式切换。若未来重新引入人工门禁（如多用户 / 团队场景），必须以新的决策档案取代本条，且权限请求必须说明将执行的动作、目标路径或命令、风险理由与作用范围，并提供与真实能力相符的选择。
- HARD / SOFT 风险提示、Goal / Team 控制、失败、重试与可验证执行结果形成可回放的本机事件。重启只恢复 Work Map、脱敏摘要、可见事件与结构化 Report，不伪装恢复已经销毁的 Worker Runtime。
...
- Project 名称与唯一项目目录关系是 D Code 自有组织元数据，与 Pi 会话权威严格分离；只有明确目录迁移通过 Host 改写关联 Header `cwd`。
...
### 最近会话、Project 与 Project Directory
...
- [ ] 用户可以创建 Project 并选择唯一 Project Directory；Project 会按更新时间平铺显示该目录精确匹配的既有 Pi Session，不出现目录会话分组。普通会话行只常驻单行标题，项目目录等完整资料在悬停或键盘聚焦时显示。
...
- [ ] 改 Project Directory 时，默认只把精确匹配关联会话的 Header `cwd` 原地迁移，Session ID、历史、谱系与 JSONL 文件路径不变；目录内文件是否同行由独立开关决定。
- [ ] 新建会话只选择 Project，新的 `cwd` 与该 Project 的唯一目录一致。

### 搜索与恢复

- [ ] 全局搜索能够命中可见会话集合中、首屏 10 条之外的历史会话标题、用户消息和助手消息，不命中未关联的旧 Pi Session。
- [ ] 搜索结果展示足以区分同名会话的 Project、项目目录、`cwd`、时间与命中片段，并支持按 Project 缩小范围。
...
- [ ] Session 与 Project 作用域中的信息检查器内容严格分开；点击 Project 无需先打开 Session 即可看到其唯一项目目录的 Files，且不改变主工作区当前会话、草稿、上下文或 Git 状态。
...
- [ ] Composer 控制行只保留 `+`、推理强度与发送；不显示 CPU 图标、模型名、模型下拉或极速开关。`+` 只提供文件、图片、Skill、命令、目标与计划，且不出现插件、扩展、本机资源或网页预览。Context 默认蓝环表示剩余容量，白色覆盖按已用比例增长，展开后显示已用量、总容量与剩余比例。
...
- [ ] 草稿没有自动过期；退出应用、切换上下文以及移除 Project 或迁移 Project Directory 都不清除草稿。发送成功或用户明确清空时只清除目标草稿；只有对应 Pi Session 被实际删除时才清除该会话的全部本机草稿。
...
- [ ] 已完成工作轮在最终助手回复下方常驻显示完成状态、完成日期时间、耗时与本轮 Pi Assistant Message 已持久化 `usage.totalTokens` 合计；缺少真实字段时省略而不估算。复制、编辑并重走、从这里继续等操作仍按悬停或键盘聚焦渐进出现，状态行始终不引发布局跳动，VoiceOver 可完整读取；稳定完成和普通中止不再在 Interaction Dock 重复显示。
- [ ] 运行中 Thinking 跨 Assistant / Tool 边界持续可见，并可与当前工具、流式正文同时呈现；完成后展开一次“中间过程”即可直接阅读思考。运行中 Composer 可在 Pi steer 的“立即介入”和 D Code 耐久 Follow-up Queue 的“排队等待”之间切换，两者的时序、恢复与数据所有权明确不同。
...
- [ ] D Team 只能显式启动，短任务仍可保持单 Agent；团队概览能显示成员职责、实际工具集合与声明范围、排队 / 运行 / 等待 / 已报告 / 失败、当前活动、耗时 / 用量、卡住 / 重试、经主智能体中转的发现 / 请求 / 指令、Verifier 退回原因与结构化报告，同时不暴露隐藏推理、完整 Worker Transcript 或凭据。
...
- [ ] 模型设置主目录只按已认证供应商展示 Pi 当前模型与刷新 / 缓存状态；未认证供应商只在底部关联入口显示身份和认证方式，不展开内建模型。“加入全局启用”只改变 Composer 选择器和快捷切换范围，并区分精确规则与通配规则；普通界面不提供手写规则输入框。单模型操作不得静默展开或删除控制多个模型的通配规则；关闭当前会话正在使用的模型也不会改写该 Session 的当前或历史模型事实，下一次显式选择才改变运行模型。
- [ ] 默认模型与启用范围经 Pi `settings.json` 合同持久化并可跨重启恢复；在线刷新失败时保留最后可用目录并明确标注状态，不把空结果伪装成供应商没有模型。
- [ ] Provider 认证入口与持久状态来自 Pi `ModelRuntime.login`；API Key / Provider 配置值只在一次 SecureField / 本机 IPC 交互中短暂存在并立即清除，D Code 不保存、不回显、不写日志，OAuth token 与认证文件正文不进入 Swift。自定义供应商和模型另经 Pi `models.json` 合同校验、原子写入与重载；合法定义进入正确供应商分组，非法定义显示可定位错误并保留上一份可用配置。
- [ ] 任一需要模型调用的 D Code 自有能力都以同一 Pi Agent Loop 中真实注册的结构化工具出现，并能从工具调用回到真实执行结果；仅在 Prompt 中列出名称不算接通。纯观察、策略或界面能力可以不注册工具。
- [ ] Agent Profile 与 Member Run 的职责、实际模型、工具集合、Skill 身份 / 版本和声明写入范围可区分查看；这些记录不冒充运行时 Permission Grant，后续配置变化不会静默改写历史。
- [ ] D Code 与 Pi CLI 均按 ADR 0023 固定完全访问：界面不出现权限卡、授权表、权限审计或模式开关，也不把声明范围伪装成技术门禁；若未来重引人工门禁，必须先由新 ADR 取代 0023。
- [ ] Goal / Team 控制、失败、重试与可验证结果可从脱敏本机事件恢复；重启不会伪装恢复 Worker Runtime，也不会持久化隐藏 Thinking、完整成员 Transcript 或凭据。
