...
- Composer 控制行前置 `+` 一次性资源调用入口：菜单按 命令 / Skill / Prompt 模板 分组列出 Pi 真实加载的资源（与 设置 > 本机资源 同源），选择把 `/…` 调用写入草稿并聚焦——只预填不发送。设置 > 自定义供应商 经 Pi `models.json` 合同管理供应商与模型定义：凭据只写不回显、既有值由 Host 合并保留、非法输入字段级报错且零写入。
