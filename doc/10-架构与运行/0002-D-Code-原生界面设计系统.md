...
- 会话栏默认 `286pt`，可在 `240–420pt` 内调整；信息检查器默认 `400pt`，可在 `400–520pt` 内调整，保证完整 Session ID 在标准字号下保持单行。旧于该下限的已保存检查器宽度在布局时自动校准。宽度本机保存，拖动内侧边缘、双击恢复默认。该边缘只是透明命中区：常态、Hover（悬停）、拖动与键盘聚焦都不显示竖线、胶囊或系统蓝色焦点框，只通过水平调宽光标表达可拖动性。
