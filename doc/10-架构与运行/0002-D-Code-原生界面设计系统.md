...
- Task HUD（任务浮层）在宽屏的局部几何是 `304pt` 宽、距主工作区顶 `66pt`、右 `24pt`、底至少 `32pt`，首选最大高 `620pt`；超出的分区只在卡片内部滚动。阅读与 Composer 在布局开始前分别保留右侧 `374pt`、左侧 `72pt` 安全区。它因此有可见的上下外缘，不得因内容变多拉成整高第三栏。
...
- HTML 文件标签是”编辑缓冲区 | 即时预览”并排双栏（0.0.18，ADR 0026）：打开即进入编辑，无源码 / 预览切换，窄窗口上下堆叠；预览为隔离 WKWebView（nonPersistent 存储），内容从内存缓冲区注入并随编辑停顿自动刷新，内联脚本默认运行；网络默认阻断——外部导航触发”本次允许 / 保持阻止”询问条（仅当前文件预览会话有效），子资源以常驻状态呈现；本机相对资源经 `dcode-asset://` 限定在项目目录授权根内。其余非 Markdown / HTML 文件维持只读行呈现，不提供切换与编辑。
...
- Round disclosure 折叠态是逐步安静摘要：相邻同类步骤合并成一行次级色状态（如「探索 · 3 文件」「已编辑 文件名 目录 +1 −1」「思考过程 持续了 4 秒」「运行命令 · 2 次」），编辑 / 创建按目标文件聚合、不同目标不合并；失败步骤在行内如实标注。摘要行数封顶（8 行），超出以「另有 N 步」收口；入口明确写“查看思考与工具”，展开后显示 Pi 已持久化并明确提供给界面的 Thinking 正文和工具过程。最新一轮从流式转为持久化时自动展开一次，用户手动收起后不因刷新反复展开。
...
- Composer 固定在 transcript 下方，以共享 raised surface 悬浮于阅读画布。控制行保留 `+`、当前模型名、独立推理强度、按模型能力出现的独立速度菜单与发送；上下文圆环只在已创建会话——存在运行上下文——时显示，会话前草稿不显示空环。模型名本身是可点击的原生 Menu，不显示 CPU / 芯片图标或外露下拉 glyph；速度只在 Host 证明支持 Fast Mode 的 OpenAI 模型显示“标准 / 极速”，不进入旧组合参数控件。普通 Run 与工具执行期间三项设置仍可调整并作用于下一安全模型边界；运行冲突不得吞掉草稿。
- 主工作区没有会话打开时，主页即会话前草稿：同一 Composer 组件居中（最大约 `640pt`）落地，启动、新建会话、打开会话与返回工作台时输入框自动获得键盘焦点；首次发送非空正文才创建真实 Pi Session（延迟创建是产品内部机制，不在界面上解说）。没有 Project 时先显示创建 Project 的空态，不以用户目录制造隐式会话。主页不显示问候语、建议 prompt 或模板卡片。
- 会话前草稿的 Composer 带作用域托盘：主输入面下方以独立阴影承托一行 Project 选择。点击该行只打开内部 Project 菜单；行内只显示项目图标与项目名称，不显示目录、分支、`~`、下拉 glyph 或“此项工作归属”。
...
- 发送按钮遵循 4.1 图标动作几何（`28pt` 视觉 / `32pt` 命中区），容器常在、权重分档：可发送时 accent 圆形填充；暂不可发送时保留同尺寸的中性极低填充圆盘（约 `0.07`）与次级色 glyph。禁用态不得退化成没有容器的裸 glyph——那会让主动作在控制行里比旁边的状态指示更弱，用户根本找不到按钮在哪；也不得回到 `36pt` 实心灰盘那种读起来像"可以按"的重量。禁用观感由按钮自己的 style 表达，不依赖系统对 disabled label 的二次变暗。模型名与推理强度都使用 caption、以相邻的独立文本 Menu 保持可见与可操作；模型名使用主文字层级，推理强度使用状态强调色。
- `/` 统一命令面板（0.0.20，ADR 0028 / 0034）：单一面板混排运行时命令（getCommands）、命令、Skill 与 Prompt 模板（resources.list，同名以 getCommands 为准去重），行尾只标注命令 / 技能 / 模板；扩展只是命令的内部来源，不成为 Composer 的产品标签。Skill 行显示 `507-explore` 这类普通名称与说明，不显示内部 `/skill:` 路由；描述单行常显、悬停 help 显示完整描述。面板可滚动（显示上限 12 行、最高约 7 行高度）；↑↓ 选择、高亮、Esc 关闭与回车选中，交互模式与 ⌘K 搜索浮层一致；插入文本复用 0.0.16 `composerInvocationText` 合同——只预填不发送。
- Composer 控制行前置 `+`（ADR 0033 / 0034）：纯 glyph 的原生 Menu，不携带菜单下拉指示符，也不使用带标题、固定宽度或胶囊按钮的自绘 Popover。菜单按“添加文件或图片 / 技能子菜单 / 命令子菜单 / 分隔 / 目标 / 计划”组织。图片经系统文件选择器或 Composer 内 Cmd-V 成为同一种附件 chip（文件名 + 体积 + 移除，≤8 张、单张原始字节 ≤5MB，超限给出用户可读原因）；剪贴板多种图片表示只消费一次，纯文本粘贴不变。图片随下一条 prompt / steer 经协议 `images` 真实进入模型输入；附件仅内存持有、不进本机 store 与后续消息队列，请求失败随正文一起恢复，受理后与会话切换时清空；非图片文件插入精确路径引用（ADR 0024）。`+` 不出现插件、扩展、本机资源、网页预览或浏览器控制，目标与计划不暴露能力来源；资源管理仍在 设置 > 本机资源。
...
## Web 迁移候选对照（0.0.30，待人工验收）

Swift 设计合同继续作为迁移基线；Web 共享数值由 `client/src/renderer/src/style.css` 拥有。Web 与 Swift 共用 [C-A 黑白变色龙品牌母版](../../app/Resources/README.md)，系统图标带浅色底板，界面品牌标志透明并随主题变色。导航行动 / 项目 / 任务行采用 36px 行高、图标操作采用 32px 命中区，阅读画布上限 760px。选中不再自动展开子会话；项目与子会话展开由明确的 disclosure 控制。

任务概览按主页面最新合同由顶栏按需打开；详情仅由具体记录进入，关闭恢复阅读空间，不叠加原浮卡留白。窄窗详情覆盖阅读区并提供关闭。用户消息为紧凑气泡，工具过程默认折叠，回答保持开放阅读面；复制 / 引用在悬停或键盘焦点时出现并保持占位。浮层仅做 160ms 透明度 / 6px 位移，减少动态效果开启时取消动画。

这些是候选实现及复验依据，尚未由 507 视觉验收，不覆盖未迁移面的 Swift 规范。

设置恢复候选沿用本文件既有设置外壳：替换日常导航的同窗分类、共享左栏宽度、约 780px 正文、语义分组与整行控件。模型等长列表按来源折叠；构建日志与回执事件按需展开，主要恢复动作保持可见。完整分类及功能恢复清单由 PRD 0028 记录，不以分类数量证明人工验收完成。


会话阅读几何与导航（2026-09-06 最新确认）：右侧任务概览关闭时，正文、空态、输入区与回到最新按钮共用居中阅读列；打开概览才向左让出位置，关闭后恢复居中。概览已经打开时，其内部内容更新不改变阅读列。宽窗详情栏并列时，阅读列在剩余空间内居中；窄窗沿用覆盖呈现，保留可读宽度。新任务入口继续保持标题与输入区相邻居中。这取代此前“概览开关不移动阅读列”的候选规则。

左侧 Conversation Rail（对话导航条）按用户消息划分轮次，工具和思考不生成轮次。沿用 Swift 的连续命中区与键盘语义：鼠标预览/点击，上下选择、Home/End 到首尾，回车或空格跳转，Escape 退出预览。当前阅读轮次根据正文锚点更新。明确跳转历史时暂停流式自动跟随；回到最新或主动滚动到底部后恢复。导航仅消费已有消息，当前位置是呈现状态，不新增 Host 协议或产品事实。


### Web 执行过程与受管附件（2026-09-06）

会话按用户提交分轮次。每轮执行过程默认一行，实时摘要来自实际思考文本或工具最新输出；展开显示按序的思考、中途说明、工具参数与结果，最终回答留在阅读面。空思考字段不形成空框；没有可展示记录时明确说明，不补造过程。

Composer 的文本区自动增高到上限后滚动，不提供浏览器 resize 手柄；键盘焦点反馈作用于输入区容器，按钮保留独立焦点。图片、文件选择及拖入/粘贴走同一附件处理路径，图片用缩略图，文件用类型和文件名卡片；附件可分别移除、点击预览。受管附件的预览使用 macOS 快速查看，历史内联图片保留放大查看入口。保留周期及过期状态由核心决定，界面不维护另一套文件寿命。

### Web 输入焦点与灵感（2026-09-06）

所有文本输入、搜索、表单与多行编辑器共用中性焦点色，不显示浏览器默认或全局样式强制生成的蓝色矩形 outline。单个表单控件显示中性边框，组合输入只由外层容器显示焦点；键盘、自动聚焦与鼠标进入都可辨认当前输入位置。文本选择和复选框的选中状态保持原语义，多行输入不提供手动缩放手柄。此规则已进入根 AGENTS.md；`client/test/layout/input-focus.mjs` 自动扫描当前控件定义并检查实际构建 CSS，实际窗口的操作路径另行走查。

“灵感”位于导航区“搜索”下方，作为同窗工作台页面；进入画布不丢失当前任务，顶栏“返回任务”回到原任务。节点选中后详情占用共享信息栏位；窄窗在工作区内覆盖显示并可关闭。文字、图片、链接和视频共用标题、正文与来源结构，媒体可经系统快速查看预览。保存、归档、恢复和任务引用均由真实核心结果确认，不显示模拟协作活动。画布布局、正文版本和任务引用分别拥有明确写入动作。

图片节点按图片原始比例自适应宽高；只作等比缩放，不裁切、不拉伸，也不固定成统一高度的缩略图槽。长图、横图、方图和小图均完整显示，点击可放大。节点实际边界统一用于连线、成组、框选、定位与“适应全部”，新建节点避开已有内容。

画布工具显示“选择、平移、框选、连线、成组”的可见名称。选择工具后立即显示当前模式、快捷键与下一步操作；平移不改变节点选择或位置，框选从节点上开始也不得变成拖动节点，Shift 可追加选择。连线逐步提示起点与终点并显示待连接线，保存完成后反馈结果；既有双向记录取消时一并处理。灵感不足两条时，连线与成组明确提示前置条件；可见内容足够但尚未多选时，成组引导进入框选。Esc 返回选择，空格临时平移在松键或失焦时解除。

## 工作台 UI/UX 并行候选

507 已授权的新任务创造页、主题扩散及后续反馈动效，只在[0030 并行候选](../40-版本实施方案/0030-工作台-UI-UX-并行候选.md)中实施和验收。新任务使用当前品牌与真实草稿；装饰场景不进入灵感或已创建任务，静态降级不影响输入；主题按钮是扩散原点。既有栏位几何、焦点、产品状态优先级保持。该候选不证明固定 0.0.31 基线已具备这些界面。
