...
会话阅读几何与导航（2026-09-06）：有消息的工作区使用共享左侧起点，留白随可用列宽在 48–92px 内变化，不再将固定宽度正文居中到整个工作区。正文、输入区和回到最新按钮共用这一列；概览的显示状态不参与正文定位。详情区在可用空间不足时沿用覆盖呈现，宽窗并列时为正文保留可读宽度。新任务入口继续保持标题与输入区相邻居中。
