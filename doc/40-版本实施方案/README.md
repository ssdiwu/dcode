...
|---|---|---|---|
| [0030-0.0.32 工作台 UI/UX 产品需求](0030-工作台-UI-UX-并行候选.md) | 授权实现已分批收口到 `27cb569`；完整候选已打包并完成临时独立窗口走查，旧窗口保留，等待 507 退出旧候选后通过新入口验收；各批证据与实际构建版本见该文档 | 目标 0.0.32，未定版、未整体人工验收、未合并或发布 | 新任务与主题、输入反馈、共享动效、技能浮层、行内 API 连接与既有 OAuth、可设置配额门槛、输入栏入口及已确认的原生证据查询缺口修复。固定 0.0.31 验收基线不替换，多目录项目后续待排期。 |
