...
|---|---|---|---|
| [0030-0.0.32 工作台 UI/UX 产品需求](0030-工作台-UI-UX-并行候选.md) | 登录修复已完成至 `79cbe21`；项目行 `7d9d120`、左右文件查看 `5181b4f`、主动展开/阅读恢复 `89c28fe` 均已提交并通过必要自动验证与独立审查；已归并 `main` 并同步远端；`cfd55ae` 固定候选完成打包与隔离启动检查，本地/远端仅保留 main | 目标 0.0.32，真实授权及整体验收待完成，未定版或发布 | 新任务与主题、输入反馈、共享动效、技能、API/OAuth 与钥匙串恢复、可设置配额门槛、原生证据、项目行及文件查看/展开。多目录项目后续待排期。 |
