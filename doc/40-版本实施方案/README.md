...
|---|---|---|---|
| [0030-0.0.32 工作台 UI/UX 产品需求](0030-工作台-UI-UX-并行候选.md) | 完整认证辅助程序生命周期已由 `5a2ab77` 修复；OpenAI Codex 两个直接入口与设备码私密投影完成源码和自动验证，等待协调任务打包新候选；历史数据边界见 PRD | 目标 0.0.32，真实钥匙串及整体验收待完成，未定版、未合并或发布 | 新任务与主题、输入反馈、共享动效、技能浮层、API/OAuth 与钥匙串访问恢复、可设置配额门槛、输入栏及原生证据修复。多目录项目后续待排期。 |
