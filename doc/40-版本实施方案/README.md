...
|---|---|---|---|
| [0028-0.0.30 Web 客户端初版产品需求](0028-0.0.30-Web客户端初版产品需求.md) | In Progress（任务二客户端骨架已落地 `d98323e`，2026-09-05；范围已冻结，[ADR 0044](../决策档案/0044-Web客户端与桌面壳技术栈边界.md) 仍 Proposed） | 待实现 | Electron + React 壳消费既有 `host/` 与 Protocol v1，落地三区布局、浮动梗概（进度 + 团队）与详情侧栏，验证打包链与动效基线；Swift 按面替换、验证即删。 |
