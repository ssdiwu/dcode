...
`0.0.17`（`837af03`）、`0.0.18`（`617249e`）、`0.0.19`（`8f0c319`）与 `0.0.20` 已进入 `main`；`0.0.25`、`0.0.26`、`0.0.27` 已分别形成公开源码标签。Published Source 基线仍是 `v0.0.27` / `main@68237c3` / `origin/main`，其实现提交为 `8403710`。当前 checkout manifest 已统一为 `0.0.28`；2026-08-25 记录 Host `220/220`、Swift `338/338` 自动测试通过，并完成同一 Team 两个 Child Provider 请求真实重叠、Provider 失败耐久状态、Coordinator Stop、Team Start / Stop 幂等重放、Coordinator 主对话先行后再启动 Team 的同一 Run / Runtime 复用、Git worktree 隔离、Task Context Selection 的 v1 → v2 私有备份晋升、选中项目 / 全局知识来源的 Prompt 回执与实收 Prompt / Receipt 同源验证、导入 Pi 会话的有界独立续接、D Code Model Catalog / Credential Reference / Runtime Model Selection、D Code Session Presentation、被选 Scope Document 无法由冻结 Git revision 物化时零 worktree / 零 Runtime 拒绝，以及 Plan / Work List 的版本化 CRUD、重排和重启恢复。它不等于干净 App 候选、真实已认证 Provider 交接、原生界面人工验收、tag、GitHub Release、签名分发或发布。
...
|---|---|---|---|
| [0027-0.0.29 任务工作台与协调者协作体验产品需求](0027-0.0.29-任务工作台与协调者协作体验产品需求.md) | Ready after `0.0.28` manual acceptance | Foundation contract candidate available；Not Implemented | 复用 Foundation Console 的生产合同，将真实 User Home / Project → Task → Coordination / Child Sessions、并行 Run、等待与 Artifact 呈现为协调者主对话、会话树、常驻 Task HUD 和具体对象 Inspector。 |
| [0026-0.0.28 D Code 原生产品数据与多会话运行基础设施产品需求](0026-0.0.28-D-Code-原生产品数据与多会话运行基础设施产品需求.md) | Implementation Candidate from `v0.0.27` | Automated + Local Dirty Candidate Passed；Clean Candidate / Native Manual / Real Auth Pending | Product Store、旧资料晋升、原生 Task / Session / Run、Pi 单向导入、多 Runtime、Coordinator 两阶段协作、D Code Prompt / Tools、Attempt / Request 与 Foundation Console 已接通；schema 不支持降级到 0.0.27。 |
