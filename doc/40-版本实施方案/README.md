...
|---|---|---|---|
| [0030-0.0.32 工作台 UI/UX 产品需求](0030-工作台-UI-UX-并行候选.md) | `5a2ab77` 生命周期修复与 `79cbe21` 两个直接登录入口已提交、通过独立审查并完成固定候选打包；实际窗口显示两种入口，完整 helper 哈希与生命周期测试一致；等待用户使用正式数据验收；项目行操作与文件查看已确认规格、实现未开始 | 目标 0.0.32，真实授权及整体验收待完成，未定版、未合并或发布 | 新任务与主题、输入反馈、共享动效、技能浮层、API/OAuth 与钥匙串访问恢复、可设置配额门槛、输入栏及原生证据修复；项目行入口、左侧文件导航、右侧内容及主动展开待实施。多目录项目后续待排期。 |
