...
|---|---|---|---|
| [0030-0.0.32 工作台 UI/UX 产品需求](0030-工作台-UI-UX-并行候选.md) | 钥匙串修复 `e450142` 已完成源码、自动回归、独立审查及固定目录候选启动检查；旧临时验收目录当前缺失，不能宣称已恢复；新入口明示使用现有正式数据并先备份，详见 PRD 交付记录 | 目标 0.0.32，真实钥匙串及整体验收待完成，未定版、未合并或发布 | 新任务与主题、输入反馈、共享动效、技能浮层、API/OAuth 与钥匙串访问恢复、可设置配额门槛、输入栏及原生证据修复。多目录项目后续待排期。 |
