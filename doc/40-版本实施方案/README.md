...
|---|---|---|---|
| [0030-0.0.32 工作台 UI/UX 产品需求](0030-工作台-UI-UX-并行候选.md) | 当前已提交基准 `3d5b4be`；原生 API 输入闪退的行内连接修复进行中，输入栏入口整合先明确 PRD 再实现；各批提交与实际构建版本见该文档 | 目标 0.0.32，未定版、未整体人工验收、未合并或发布 | 新任务与主题、输入反馈、共享动效、技能浮层、行内 API 连接与既有 OAuth、可设置配额门槛；项目归属收进输入卡底部、技能入口进入“＋”。固定 0.0.31 验收基线不替换，多目录项目后续待排期。 |
