...
|---|---|---|---|
| [0030-0.0.32 工作台 UI/UX 产品需求](0030-工作台-UI-UX-并行候选.md) | OAuth 授权恢复 `b07564d` 已完成源码修复、自动验证、独立审查、候选打包与临时独立数据启动检查，包含此前 `27cb569` 全部能力；等待用户在原验收数据重试真实登录，旧窗口与数据保留，各批证据见该文档 | 目标 0.0.32，未定版、未整体人工验收、未合并或发布 | 新任务与主题、输入反馈、共享动效、技能浮层、行内 API 连接与可恢复 OAuth、可设置配额门槛、输入栏入口及已确认的原生证据查询缺口修复。固定 0.0.31 验收基线不替换，多目录项目后续待排期。 |
