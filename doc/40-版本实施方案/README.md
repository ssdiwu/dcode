...
|---|---|---|---|
| [0030-0.0.32 工作台 UI/UX 产品需求](0030-工作台-UI-UX-并行候选.md) | 原有批次完整候选基准 `27cb569`；2026-09-09 新反馈的 OpenAI Codex OAuth 授权恢复已完成源码修复、自动验证与独立审查，等待新候选；不能将 API 行内连接通过视作真实 OAuth 验收通过；旧窗口与数据保留，各批证据见该文档 | 目标 0.0.32，未定版、未整体人工验收、未合并或发布 | 新任务与主题、输入反馈、共享动效、技能浮层、行内 API 连接与既有 OAuth、可设置配额门槛、输入栏入口及已确认的原生证据查询缺口修复。固定 0.0.31 验收基线不替换，多目录项目后续待排期。 |
