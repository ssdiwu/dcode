...
|---|---|---|---|
| [0030-0.0.32 工作台 UI/UX 产品需求](0030-工作台-UI-UX-并行候选.md) | 钥匙串修复 `e450142` 已交付固定候选；完整认证辅助程序实际出现定时器崩溃，正在修复生命周期并实现 OpenAI Codex 两个直接登录入口；历史数据边界见 PRD | 目标 0.0.32，真实钥匙串及整体验收待完成，未定版、未合并或发布 | 新任务与主题、输入反馈、共享动效、技能浮层、API/OAuth 与钥匙串访问恢复、可设置配额门槛、输入栏及原生证据修复。多目录项目后续待排期。 |
